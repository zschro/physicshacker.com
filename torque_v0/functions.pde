...
int Nplanetoids = 12;
float[] x_planetoid = new float[Nplanetoids];
float[] y_planetoid = new float[Nplanetoids];
float[] vx_planetoid = new float[Nplanetoids];
float[] vy_planetoid = new float[Nplanetoids];
float[] size_planetoid = new float[Nplanetoids];



void make_planetoids() {
  
  for (int i = 0;i<Nplanetoids;i++){
...
void display() { 
    ...
    fill(255);
    ellipseMode(RADIUS); 
    float circ_radius = 25;   
    if ((ang_accel != 0) && showarrows) {
    ellipse(x,y,circ_radius,circ_radius);
    }

    float line_len_torque=10;
    if ( (ang_accel > 0) && showarrows ) {
    line(x+circ_radius,y,x+circ_radius+line_len_torque*sin(35.*TWO_PI/360),y+line_len_torque*cos(35.*TWO_PI/360));
    line(x+circ_radius,y,x+circ_radius-line_len_torque*sin(35.*TWO_PI/360),y+line_len_torque*cos(35.*TWO_PI/360));
    }
    if ( (ang_accel < 0) && showarrows) {
    line(x-circ_radius,y,x-circ_radius+line_len_torque*sin(40.*TWO_PI/360),y+line_len_torque*cos(40.*TWO_PI/360));
    line(x-circ_radius,y,x-circ_radius-line_len_torque*sin(30.*TWO_PI/360),y+line_len_torque*cos(30.*TWO_PI/360));
    }
...
//    if (((Fx != 0) || (Fy != 0)) && 0 ) {
    ...
    textSize(20);
    fill(255,0,0);
    text("Velocity",10,400);
    fill(0,0,255);
    text("Force",10,425);
    fill(204,0,204);
    text("Acceleration",10,450);
    fill(255,177,100);
    text("Torque",10,475);
    ...
    if (y_planetoid[i] < 0) x_planetoid[i] = height;
    ellipse(x_planetoid[i],y_planetoid[i],size_planetoid[i],size_planetoid[i]);
    }

    for (int i=0;i<Nplanetoids;i++){
    float dx = x - x_planetoid[i];
    float dy = y - y_planetoid[i];
